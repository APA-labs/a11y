  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className={`text-xs font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${cfg.labelClass}`}>
          {cfg.label}
        </span>
...
export default function ChecklistSection({ must, should, avoid }: Props) {
  return (
    <div className="space-y-5">
      <ChecklistGroup items={must} level="must" />
      <ChecklistGroup items={should} level="should" />
      <ChecklistGroup items={avoid} level="avoid" />
    </div>
  )
}
