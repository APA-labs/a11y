...
export default function PatternCard({ pattern, lang }: { pattern: Pattern; lang: Lang }) {
  ...
  return (
    <Link
      ...
      className='group block bg-surface rounded-xl border border-outline hover:border-violet-300 hover:shadow-sm transition-all overflow-hidden dark:hover:border-violet-700'>
      {/* 썸네일 */}
      <div
        className='relative w-full bg-canvas border-b border-divider overflow-hidden'
        style={{ height: 100 }}>
        <Image
          src={previewSrc}
      ...
      <div className='p-3'>
        <div className='flex items-start justify-between mb-1.5'>
          <div className='flex-1 min-w-0 pr-1'>
            <h2 className='font-semibold text-body text-[13px] group-hover:text-violet-600 transition-colors leading-tight'>{pattern.name}</h2>
            <p className='text-[11px] text-soft mt-0.5 leading-snug'>{pattern.description}</p>
          </div>
          <ArrowRight
            size={12}
            className='shrink-0 mt-0.5 text-faint group-hover:text-violet-600 group-hover:translate-x-0.5 transition-all'
          />
        </div>
        ...
        <div className='flex items-center justify-between pt-2 border-t border-divider'>
          <div className='flex items-center gap-2 text-[11px] text-soft'>
            <span>
              <span className='font-semibold text-red-500 dark:text-red-400'>{mustCount}</span> must
            </span>
            <span>
